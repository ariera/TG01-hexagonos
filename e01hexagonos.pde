...
void setup() {
  ...
  size(2*img.width,img.height + 100 + TITLE_HEIGHT);
  image(img,0,TITLE_HEIGHT);
  output = createImage(img.width, img.height, RGB);

  display_instructions(img.width, img.height);
  displayTitle();
}
...
void display_instructions(int base_width, int base_height){
  ...
  text("Clique sobre un pixel para ver su \nrepresentacion hexagonal\n\nClick derecho para mostrar los hexagonos" , base_width, base_height + 20 + TITLE_HEIGHT);  
}
...
void draw() {
  ...
  output.updatePixels();
  image(output,img.width,TITLE_HEIGHT);
...
void dibujarPixel(int altura, int[] loc, int promedio){
  ...
  text("Pixel Hexagonal\t\t\t\t\tPromedio" , 10, img.height + 20 + TITLE_HEIGHT);
  ...
  fill(colors[0]);
  rect(x+(w*1), y+(w*0)+TITLE_HEIGHT, w, w);
  fill(colors[1]);
  rect(x+(w*2), y+(w*0)+TITLE_HEIGHT, w, w);
  fill(colors[2]);
  rect(x+(w*0), y+(w*1)+TITLE_HEIGHT, w, w);
  fill(colors[3]);
  rect(x+(w*1), y+(w*1)+TITLE_HEIGHT, w, w);
  fill(colors[4]);
  rect(x+(w*2), y+(w*1)+TITLE_HEIGHT, w, w);
  fill(colors[5]);
  rect(x+(w*3), y+(w*1)+TITLE_HEIGHT, w, w);
  fill(colors[6]);
  rect(x+(w*1), y+(w*2)+TITLE_HEIGHT, w, w);
  fill(colors[7]);
  rect(x+(w*2), y+(w*2)+TITLE_HEIGHT, w, w);

  fill(promedio);
  rect(x+100, y+TITLE_HEIGHT, 60,60);
